---
title: "DEPI Analysis - January MPK"
...
The data was collected in December 2017, January 2018, and Februrary 2018. These different collection periods will be analyzed seperately in seperate RMD files.
...
Next, load in the data. This data is in a seperate query from the December and February data. I included all columns in both the individual_plant_metadata and the DEPI measurement files - for the other experiments, I did not include all of these columns. 
...
There are some genotypes that are not MPK mutants in this data set. Create a subset of only MPK mutants and  Col0 to use in analysis. Also, only use subline 1 of Col0.

```{r create_depi_subset}
depi_subset <- depi_data%>%filter(!genotype %in% c("b1", "b3", "b1b3", "ftsz2-1", "ftsz2-2", "ftsz-dbl", "Col0")|(genotype =="Col0" & full_subline_information %in% c("Col1_1", "Col1_3", "Col1_4", "Col1_2")))
...
jan_data <- filter(depi_subset, border == FALSE)
```

Finally, lets look at the genotype we have:

```{r genotype_invest}
unique(jan_data$genotype)
```


##Create functions

These functions will be applied to each experiment (December, January, and February) seperately. 
...
The p_value function finds the p-value for the comparison of each genotype to wildtype in order to answer the question - does phenotype (either leaf area, npq, or phi2) significantly differ from wild type? The test is repeated for each time point for each genotype.
...
```{r jan_summary_measured_value}
jan_data%>%group_by(measurement)%>%summarize(min = min(measured_value))
```
...
```{r jan_apply_functions}
jan_data <- add_day_col(add_number(jan_data))
```
...
```{r jan_outliers}
jan_outliers <- remove_outliers(jan_data)
###Number of outliers
sum(is.na(jan_outliers$measured_value))
###Proportion of measured values that are outliers
sum(is.na(jan_outliers$measured_value))/nrow(jan_outliers)

```

This proportion is similar to the propotion of outliers in the February and December data set.
...
jan_data_p <- p_value(jan_data)

###Test 1 
unique(filter(jan_data_p, genotype == "mpk1", time_point == "1", measurement == "npq")$p)
a <- filter(jan_data, genotype == "mpk1", time_point == "1", measurement == "npq")$measured_value
b <- filter(jan_data, genotype == "Col0", time_point == "1", measurement == "npq")$measured_value
wilcox.test(a, b, correct = FALSE)$p.value
###Test 2
unique(filter(jan_data_p, genotype == "mpk14-17", time_point == "0", measurement == "leafarea")$p)
c <- filter(jan_data, genotype == "mpk14-17", time_point == "0", measurement == "leafarea")$measured_value
d <- filter(jan_data, genotype == "Col0", time_point == "0", measurement == "leafarea")$measured_value
wilcox.test(c,d, correct = FALSE)$p.value
###Test 3
unique(filter(jan_data_p, genotype == "mpk13", time_point == "0", measurement == "phi2")$p)
e <- filter(jan_data, genotype == "mpk13", time_point == "0", measurement == "phi2")$measured_value
f <- filter(jan_data, genotype == "Col0", time_point == "0", measurement == "phi2")$measured_value
...
```{r jan_correct_p_values}
jan_data_p_corrected <- corrected_p_value(jan_data_p)
jan_data_p_corrected$effect_size <- factor(jan_data_p_corrected$effect_size, levels = c("small", "medium", "large"))
summary(jan_data_p_corrected$effect_size)
```

Here is the distribution of effect sizes:

```{r jan_investigate_effect}
ggplot(data = subset(jan_data_p_corrected, !is.na(effect_size)), aes(x = effect_size, na.rm = TRUE))+
  geom_bar()+
  labs(x = "Effect size", y = "Count", title = "January: Effect size for significant p-values")+
...
jan_data_p_corrected%>%group_by(measurement, type)%>%summarize(median = median(p), min = min(p), max = max(p))


```

For each measurement there are significant p-values.
...
```{r}
jan_data_p_corrected%>%group_by(type)%>%summarize(p_lessthan_0.05 = sum(p<0.05), p_lessthan_0.01 = sum(p<0.01))
```

Next, visualize p-values for January. Create a modified data frame to be used in to create visualizations.

```{r jan_create_plot_data}
jan_data_plot <- add_number(jan_data_p_corrected)
```
...
ggplot(jan_data_plot, aes(x=p, color=type)) +
  ...
  geom_text(aes(x = 0.18, label = "p=0.05", y =  3000), color = "black", size = 15)+
  geom_vline(xintercept=0.05, color = "black", size = 1)+
  theme_minimal(base_family = "Calibri",
              base_size = 50)+
  labs(x = "P-Value", y = "Count", title = "January: P-value Distribution")
...
Add bins for the p-values to easily visualize what is significant and what isn't. The minimum p-value for leafarea is less than 0.05 - this was not the case for the other experiments. 

```{r jan_plot_subsets}
###Create seperate data frames for each measurement to use in heat maps; don't use adjusted p-values
jan_npq <- filter(jan_data_plot, measurement == "npq", type == "p")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
    (p>0.05)~"p>0.05"))
jan_phi2 <- filter(jan_data_plot, measurement == "phi2", type == "p")%>%
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
    (p>0.05)~"p>0.05"))
jan_leafarea <- filter(jan_data_plot, measurement == "leafarea", type == "p")%>%
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
    (p>0.05)~"p>0.05"))
...
jan_npq$bin <- as.factor(jan_npq$bin)
jan_phi2$bin <- as.factor(jan_phi2$bin)
jan_leafarea$bin <- as.factor(jan_leafarea$bin)

###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
jan_npq$bin <- relevel(jan_npq$bin, 'p<0.01')
jan_phi2$bin <- relevel(jan_phi2$bin, 'p<0.01')
jan_leafarea$bin <- relevel(jan_leafarea$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
jan_npq$genotype <- reorder(jan_npq$genotype, jan_npq$number)
jan_phi2$genotype <- reorder(jan_phi2$genotype, jan_phi2$number)
jan_leafarea$genotype <- reorder(jan_leafarea$genotype, jan_leafarea$number)
...
```{r dec_uncorrected_p_value_heat_maps}
##### ----- Dec p-value heat map - NPQ ------
ggplot(data = jan_npq, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: NPQ P-value, Uncorrected")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =        
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,
  #135,144,159,168,183,192,207,216,231,240,255,264,279),0))+
...
ggplot(data = jan_phi2, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: Phi2 P-value, Uncorrected")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =   
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,
...
##### ----- Dec p-value heat map - leafarea ------
ggplot(data = jan_leafarea, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: Leafarea P-value, Uncorrected")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =   
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,
...
jan_npq_adj <- filter(jan_data_plot, measurement == "npq", type == "p_adj")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
    (p>0.05)~"p>0.05"))
jan_phi2_adj <- filter(jan_data_plot, measurement == "phi2", type == "p_adj")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
    (p>0.05)~"p>0.05"))
jan_leafarea_adj <- filter(jan_data_plot, measurement == "leafarea", type == "p_adj")%>%
  group_by(day)%>%
  filter(time_point %in% c(max(time_point), min(time_point)))%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
###In order to use these bins as the fill in a heat map, convert to a factor
###In order to use these bins as the fill in a heat map, convert to a factor
jan_npq_adj$bin <- as.factor(jan_npq_adj$bin)
jan_phi2_adj$bin <- as.factor(jan_phi2_adj$bin)
jan_leafarea_adj$bin <- as.factor(jan_leafarea_adj$bin)

###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
jan_npq_adj$bin <- relevel(jan_npq_adj$bin, 'p<0.01')
jan_phi2_adj$bin <- relevel(jan_phi2_adj$bin, 'p<0.01')
jan_leafarea_adj$bin <- relevel(jan_leafarea_adj$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
jan_npq_adj$genotype <- reorder(jan_npq_adj$genotype, jan_npq_adj$number)
jan_phi2_adj$genotype <- reorder(jan_phi2_adj$genotype, jan_phi2_adj$number)
jan_leafarea_adj$genotype <- reorder(jan_leafarea_adj$genotype, jan_leafarea_adj$number)
...
```{r}
jan_data_plot%>%filter(type == "p_adj")%>%group_by(measurement)%>%summarize(sum(p<0.01), sum(p>0.01 & p<0.05))
```
...
##### ----- Jan p-value heat map - NPQ ------
ggplot(data = jan_npq_adj, aes(x = time_point, y = genotype, fill = bin)) + 
...
##### ----- Jan p-value heat map - phi2 ------
ggplot(data = jan_phi2_adj, aes(x = time_point, y = genotype, fill = bin)) + 
...
##### ----- Jan p-value heat map - leafarea ------
ggplot(data = jan_leafarea_adj, aes(x = time_point, y = genotype, fill = bin)) + 
...
## Cell 371
Next, we'll transition to general visualizations for December

Apply the cell_371_data function to December, and add columns with the day, number, and number_2.

```{r jan_cell_371}
cell_371_jan <- cell_371_data(add_day_col(jan_data))
cell_371_jan <- add_number(cell_371_jan)
```
...
unique(c)
unique(filter(cell_371_jan, genotype == "mpk1", time_point == "1", measurement == "npq")$log2_fold)
###Test 2 
d <- filter(jan_data, genotype == "mpk9-16", time_point == "1", measurement == "phi2")$measured_value
e <- filter(jan_data, genotype == "Col0", time_point == "1", measurement == "phi2")$measured_value
f <- log2(median(d/median(e)))
f
unique(filter(cell_371_jan, genotype == "mpk9-16", time_point == "1", measurement == "phi2")$log2)
...
cell_371_leafarea_jan <- cell_371_jan%>%filter(measurement == "leafarea")%>%arrange(number)
cell_371_npq_jan <- cell_371_jan%>%filter(measurement == "npq")%>%arrange(number)
cell_371_phi2_jan <- cell_371_jan%>%filter(measurement == "phi2")%>%arrange(number)

###Reorder each measurement data set by the number column so Col0 is on top, followed by single, followed by double mutants
cell_371_leafarea_jan$genotype <- reorder(cell_371_leafarea_jan$genotype, cell_371_leafarea_jan$number)
cell_371_npq_jan$genotype <- reorder(cell_371_npq_jan$genotype, cell_371_npq_jan$number)
cell_371_phi2_jan$genotype <- reorder(cell_371_phi2_jan$genotype, cell_371_phi2_jan$number)

```

```{r dec_logfold_summary}
summary(cell_371_jan$log2_fold)
```

Next, plot the data. Some notes: I chose to set the color scale to be in the range of -0.7 to 0.8 for all measurements. I chose the same scale so that all three heat maps were comparable. This encompasses the range of the log fold change values for December. 

```{r jan_logfold_heat_maps}
##### ----- Jan Phi2 heat map - all genotypes ------
...
ggplot(data = cell_371_phi2_jan, aes(x = time_point, y = genotype, fill = log2_fold)) + 
  ...
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.7, .8), breaks = c(-0.7, 0, 0.8), labels = c("-0.7", "0", "0.8"))
#ggsave("dec_phi2_heatmap.png", scale = 2, path = "C:/Users/Joan Seeger/Documents/Shiu Lab - R/Current/Visualizations")

##### ----- Jan Leaf area heat map - all genotypes ------

###Confirm that each day has two "cells" - this is because we selected only start and end points for each day
ggplot(data = cell_371_leafarea_jan, aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "January: Leaf Area Log 2 Fold Change")+
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.7, .8), breaks = c(-0.7, 0, 0.8), labels = c("-0.7", "0", "0.8"))
#ggsave("jan_leafarea_heatmap.png", scale = 2, path = "C:/Users/Joan Seeger/Documents/Shiu Lab - R/Current/Visualizations")

##### ----- Jan NPQ heat map - all genotypes ------

ggplot(data = cell_371_npq_jan, aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "January: NPQ Log 2 Fold Change")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks = 
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,
...
for (element in genotype_combinations){
  for (m in c("npq", "leafarea", "phi2")){
    ...
    plot <- ggplot(data = data, aes(x = time_point, y = genotype, fill = log2_fold)) + 
      labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = paste("January:", m, "Log 2 Fold Change"))+
...
Apply the cell_370_data_function to the December data, and add columns with the day, number, and number_2.

```{r jan_cell_370}
cell_370_data_jan <- cell_370_data(add_day_col(jan_data))
cell_370_data_jan <- add_number(cell_370_data_jan)
cell_370_data_jan <- add_day_col(cell_370_data_jan)

cell_370_data_jan$genotype <- reorder(cell_370_data_jan$genotype, cell_370_data_jan$number)

```

```{r dec_cell_370_loop}
for (element in genotype_combinations){
  data <- filter(cell_370_data_jan, genotype %in% c(element, "Col0"))
