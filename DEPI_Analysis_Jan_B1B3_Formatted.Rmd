...
library(knitr)
library(formatR)
knitr::opts_chunk$set(warning=FALSE, message=FALSE, dev = "cairo_pdf", tidy.opts=list(width.cutoff=20),tidy=TRUE)
```
...
jan_b1b3 <- depi_data%>%filter(genotype %in% c("b1", "b3", "b1b3", "Col0" )|(genotype == "Col0"&subline == "1"))
...
jan_b1b3_npq <- filter(jan_b1b3_plot, measurement == "npq", type == "p")%>%
...
jan_b1b3_npq$bin <- as.factor(jan_b1b3_npq$bin)
...
jan_b1b3_npq$bin <- relevel(jan_b1b3_npq$bin, 'p<0.01')
...
jan_b1b3_npq$genotype <- reorder(jan_b1b3_npq$genotype, jan_b1b3_npq$number)
...
ggplot(data = jan_b1b3_npq, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: NPQ P-value, Uncorrected, for B1B3")+
...
jan_b1b3_npq_adj <- filter(jan_b1b3_plot, measurement == "npq", type == "p_adj")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
###Reorder by number so heat map has WT first, then single, then double mutants
jan_b1b3_npq_adj$genotype <- reorder(jan_b1b3_npq_adj$genotype, jan_b1b3_npq_adj$number)
...
##### ----- Jan B1B3 p-value heat map - NPQ ------
ggplot(data = jan_b1b3_npq_adj, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwith FDR Correction", x = "Hours", y = NULL, title = "January: NPQ P-value, Corrected, for B1B3")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
...
cell_371_leafarea_jan_b1b3 <- cell_371_jan_b1b3%>%filter(measurement == "leafarea")%>%arrange(number_2)
cell_371_npq_jan_b1b3 <- cell_371_jan_b1b3%>%filter(measurement == "npq")%>%arrange(number_2)
cell_371_phi2_jan_b1b3<- cell_371_jan_b1b3%>%filter(measurement == "phi2")%>%arrange(number_2)
...
Next, plot the data. Some notes: I chose to set the color scale to be in the range of -0.5 to 0.4 for all measurements. I chose the same scale so that all three heat maps were comparable. This encompasses the range of the log fold change values for December. 
...
ggplot(data = filter(cell_371_phi2_jan_b1b3, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  ...
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.5, 0.4), breaks = c(-0.5, 0, 0.4), labels = c("-0.5", "0", "0.4"))
...
```{r jan_b1b3_cell_370}
cell_370_data_jan_b1b3 <- cell_370_data(add_day_col(jan_b1b3))
