---
title: "DEPI Analysis - February B1B3"
...
This data was collected in February 2018. Additional collection periods include December 2017, January 2018, and March 2018.

We will analyze each collection period seperately. This is the analysis for the February experiment for the B1B3 family. There are additional files for MPK and Ftsz analysis. 
...
Next, load in the data. This data includes all gene families - we will later filter to only select B1B3, B1, and B3 genotypes. This data file includes both the December and February data. 
...
Add column with full subline information:
...
feb_b1b3 <- depi_data%>%filter(genotype %in% c("b1", "b3", "b1b3" )|(genotype =="Col0"&full_subline_information %in% c("Col1-1", "Col1-3", "Col1-4", "Col1-2")))

###Confirm we have the correct sublines:

unique(filter(feb_b1b3, genotype == "Col0")$full_subline_information)
...
levels(feb_b1b3$measurement)[levels(feb_b1b3$measurement)=="growth"] <- "leafarea"
levels(feb_b1b3$measurement)[levels(feb_b1b3$measurement)=="size"] <- "leafarea"
...
feb_b1b3 <- filter(feb_b1b3, border == FALSE, substr(plant_ID, 1,4) == "0218")
```

Finally, lets look at the genotypes we have in this subset:

```{r genotype_invest}
unique(feb_b1b3$genotype)
```
...
```{r feb_b1b3_summary_measured_value}
feb_b1b3%>%group_by(measurement)%>%summarize(min = min(measured_value))
```
...
```{r feb_b1b3_shift_npq}
feb_b1b3$measured_value[feb_b1b3$measurement == "npq"] <- (feb_b1b3$measured_value[feb_b1b3$measurement == "npq"])+abs(min((filter(feb_b1b3, measurement == "npq"))$measured_value))
...
```{r feb_b1b3_apply_functions}

feb_b1b3 <- add_number(feb_b1b3)
feb_b1b3 <-add_day_col(feb_b1b3)
```
...
```{r feb_b1b3_outliers}
feb_b1b3_outliers <- remove_outliers(feb_b1b3)
###Number of outliers
sum(is.na(feb_b1b3_outliers$measured_value))
###Proportion of measured values that are outliers
sum(is.na(feb_b1b3_outliers$measured_value))/nrow(feb_b1b3_outliers)
...
feb_b1b3_p <- p_value(feb_b1b3)

###Test 1 
unique(filter(feb_b1b3_p, genotype == "b1", time_point == "1", measurement == "npq")$p)
a <- filter(feb_b1b3, genotype == "b1", time_point == "1", measurement == "npq")$measured_value
b <- filter(feb_b1b3, genotype == "Col0", time_point == "1", measurement == "npq")$measured_value
wilcox.test(a, b, correct = FALSE)$p.value
###Test 2
unique(filter(feb_b1b3_p, genotype == "b1b3", time_point == "1", measurement == "leafarea")$p)
c <- filter(feb_b1b3, genotype == "b1b3", time_point == "1", measurement == "leafarea")$measured_value
d <- filter(feb_b1b3, genotype == "Col0", time_point == "1", measurement == "leafarea")$measured_value
...
```{r feb_b1b3_correct_p_values}
feb_b1b3_p_corrected <- corrected_p_value(feb_b1b3_p)
feb_b1b3_p_corrected$effect_size <- factor(feb_b1b3_p_corrected$effect_size, levels = c("small", "medium", "large"))
summary(feb_b1b3_p_corrected$effect_size)
```

Here is the distribution of the effect sizes - note that there are no "large" effect sizes, and no effect sizes are inf.  

```{r feb_b1b3_investigate_effect}
ggplot(data = subset(feb_b1b3_p_corrected, !is.na(effect_size)), aes(x = effect_size, na.rm = TRUE))+
  geom_bar()+
  labs(x = "Effect size", y = "Count", title = "February: Effect size for significant p-values, for B1B3")+
...
```{r feb_b1b3_summary_p}
###Summary statistics for the p values for each measurement;leaf area is never significant
feb_b1b3_p_corrected%>%group_by(measurement, type)%>%summarize(median = median(p), min = min(p), max = max(p))
...
feb_b1b3_p_corrected%>%group_by(type)%>%summarize(p_lessthan_0.05 = sum(p<0.05), p_lessthan_0.01 = sum(p<0.01))

```

Next, we will visualize p-values for February for B1B3. Create a modified data frame to be used in to create visualizations.

```{r feb_b1b3_create_plot_data}
feb_b1b3_plot <- add_number(feb_b1b3_p_corrected)

#feb_b1b3_plot$day <- as.integer(feb_b1b3_plot$day)
...
ggplot(feb_b1b3_plot, aes(x=p, color=type)) +
  ...
  theme_minimal(base_family = "Calibri",
              base_size = 50)+
  labs(x = "P-Value", y = "Count", title = "February: P-value Distribution, for B1B3")
...
```{r feb_b1b3_modify_bins}

###In order to use these bins as the fill in a heat map, convert to a factor
feb_b1b3_npq$bin <- as.factor(feb_b1b3_npq$bin)


###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
feb_b1b3_npq$bin <- relevel(feb_b1b3_npq$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
feb_b1b3_npq$genotype <- reorder(feb_b1b3_npq$genotype, feb_b1b3_npq$number)

#feb_b1b3_npq$day <- as.integer(feb_b1b3_npq$day)
...
```{r feb_b1b3_uncorrected_p_value_heat_maps}

##### ----- Jan B1B3 p-value heat map - NPQ ------
ggplot(data = feb_b1b3_npq, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "February: NPQ P-value, Uncorrected, for B1B3")+
...
```{r feb_b1b3_plot_modify_bins_adj}
###In order to use these bins as the fill in a heat map, convert to a factor
feb_b1b3_npq_adj$bin <- as.factor(feb_b1b3_npq_adj$bin)

###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
feb_b1b3_npq_adj$bin <- relevel(feb_b1b3_npq_adj$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
feb_b1b3_npq_adj$genotype <- reorder(feb_b1b3_npq_adj$genotype, feb_b1b3_npq_adj$number)

#feb_b1b3_npq_adj$day <- as.integer(feb_b1b3_npq_adj$day)

```


```{r}
feb_b1b3_plot%>%filter(type == "p_adj")%>%group_by(measurement)%>%summarize(sum(p<0.01), sum(p>0.01 & p<0.05))
```

Create heat maps for the adjusted p-values.

```{r jan_p_value_heat_maps_adj}
##### ----- Feb B1B3 p-value heat map - NPQ ------
ggplot(data = feb_b1b3_npq_adj, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwith FDR Correction", x = "Hours", y = NULL, title = "February: NPQ P-value, Corrected, for B1B3")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
...
## Cell 371
Next, we'll transition to general visualizations for February for the B1B3 genotypes. 

Apply the cell_371_data function to the B1B3 February subset, and add columns with the day, number, and number_2.

```{r feb_b1b3_cell_371}
cell_371_feb_b1b3 <- cell_371_data(add_day_col(feb_b1b3))
cell_371_feb_b1b3$genotype <- as.character(cell_371_feb_b1b3$genotype)
cell_371_feb_b1b3 <- add_number(cell_371_feb_b1b3)
#cell_371_feb_b1b3$day <- as.integer(cell_371_feb_b1b3$day)

```

Next, we want to verify that our function is giving us the correct log fold values.

```{r feb_b1b3_verify_median}
###Test 1 
a <- filter(feb_b1b3, genotype == "b1b3", time_point == "1", measurement == "npq")$measured_value
b <- filter(feb_b1b3, genotype == "Col0", time_point == "1", measurement == "npq")$measured_value
c <- log2(median(a/median(b)))
unique(c)
unique(filter(cell_371_feb_b1b3, genotype == "b1b3", time_point == "1", measurement == "npq")$log2_fold)
###Test 2 
d <- filter(feb_b1b3, genotype == "b1", time_point == "1", measurement == "phi2")$measured_value
e <- filter(feb_b1b3, genotype == "Col0", time_point == "1", measurement == "phi2")$measured_value
f <- log2(median(d/median(e)))
f
unique(filter(cell_371_feb_b1b3, genotype == "b1", time_point == "1", measurement == "phi2")$log2)
...
```{r feb_b1b3_cell_371_subsets}
cell_371_leafarea_feb_b1b3 <- cell_371_feb_b1b3%>%filter(measurement == "leafarea")%>%arrange(number_2)
cell_371_npq_feb_b1b3 <- cell_371_feb_b1b3%>%filter(measurement == "npq")%>%arrange(number_2)
cell_371_phi2_feb_b1b3<- cell_371_feb_b1b3%>%filter(measurement == "phi2")%>%arrange(number_2)

###Reorder each measurement data set by the number column so Col0 is on top, followed by single, followed by double mutants
cell_371_leafarea_feb_b1b3$genotype <- reorder(cell_371_leafarea_feb_b1b3$genotype, cell_371_leafarea_feb_b1b3$number_2)
cell_371_npq_feb_b1b3$genotype <- reorder(cell_371_npq_feb_b1b3$genotype, cell_371_npq_feb_b1b3$number_2)
cell_371_phi2_feb_b1b3$genotype <- reorder(cell_371_phi2_feb_b1b3$genotype, cell_371_phi2_feb_b1b3$number_2)

```

```{r feb_b1b3_logfold_summary}
summary(cell_371_feb_b1b3$log2_fold)
```

Next, plot the data. Some notes: I chose to set the color scale to be in the range of -0.2 to 1.0 for all measurements. I chose the same scale so that all three heat maps were comparable. This encompasses the range of the log fold change values for December. 

```{r feb_b1b3_logfold_heat_maps}
##### ----- Feb B1B3 Phi2 heat map - all genotypes ------

ggplot(data = filter(cell_371_phi2_feb_b1b3, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "February: Phi2 Log 2 Fold Change, for B1B3")+
  #geom_tile(width = 10 , height = 20)+
  ...
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.2, 1), breaks = c(-0.2, 0, 1), labels = c("-0.2", "0", "1"))
#ggsave("dec_phi2_heatmap.png", scale = 2, path = "C:/Users/Joan Seeger/Documents/Shiu Lab - R/Current/Visualizations")

##### ----- Feb B1B3 Leaf area heat map - all genotypes ------

###Confirm that each day has two "cells" - this is because we selected only start and end points for each day
ggplot(data = filter(cell_371_leafarea_feb_b1b3, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "February: Leaf Area Log 2 Fold Change, for B1B3")+
...
##### ----- Feb B1B3 NPQ heat map - all genotypes ------

ggplot(data = filter(cell_371_npq_feb_b1b3, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "February: NPQ Log 2 Fold Change, for B1B3")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
...
```{r feb_b1b3_cell_370}
cell_370_data_feb_b1b3 <- cell_370_data(add_day_col(feb_b1b3))
cell_370_data_feb_b1b3$genotype <- as.character(cell_370_data_feb_b1b3$genotype)
cell_370_data_feb_b1b3 <- add_number(cell_370_data_feb_b1b3)
cell_370_data_feb_b1b3 <- add_day_col(cell_370_data_feb_b1b3)

cell_370_data_feb_b1b3$genotype <- reorder(cell_370_data_feb_b1b3$genotype, cell_370_data_feb_b1b3$number)
...
```{r feb_b1b3_cell_370_plots}
ggplot(data = cell_370_data_feb_b1b3, aes(x = time_point, y = med))+
