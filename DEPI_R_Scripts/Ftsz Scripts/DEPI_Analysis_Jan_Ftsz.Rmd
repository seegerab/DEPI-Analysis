---
title: "DEPI Analysis - January Ftsz"
...
tidy.opts=list(width.cutoff=30),tidy=TRUE, cache = TRUE) 
...
We will analyze each collection period seperately. This is the analysis for the January experiment for the Ftsz family. There are additional files for MPK and B1B3
analysis. 
...
Next, load in the data. This data includes all gene families - we will later filter to only select Ftsz, B1, and B3 genotypes. This data file includes only the January data. 
...
We only want to analyze the Ftsz genes in this analysis. And, we only want subline 2 of Col0.


```{r create_Ftsz_subset}
jan_ftsz <- depi_data%>%filter(genotype %in% c("ftsz-dbl", "ftsz2-1", "ftsz2-2" )|(genotype =="Col0" & full_subline_information %in% c("Col2_1", "Col2_3", "Col2_4", "Col2_2")))

##Confirm that we only have subline 1 of Col0
unique(filter(jan_ftsz, genotype == "Col0")$full_subline_information)
...
levels(jan_ftsz$measurement)[levels(jan_ftsz$measurement)=="growth"] <- "leafarea"
levels(jan_ftsz$measurement)[levels(jan_ftsz$measurement)=="size"] <- "leafarea"

```

Filter to remove the border plants.

```{r create_Jan_subset}
###Create two data sets based on collection period and remove border plants 
jan_ftsz <- filter(jan_ftsz, border == FALSE)
```

Finally, lets look at the genotypes we have in this subset and confirm that we only have subline 1 of Col0:

```{r genotype_invest}
unique(jan_ftsz$genotype)
unique(filter(jan_ftsz, genotype =="Col0")$subline)
```
...
```{r jan_ftsz_summary_measured_value}
jan_ftsz%>%group_by(measurement)%>%summarize(min = min(measured_value))
```
...
```{r jan_ftsz_shift_npq}
jan_ftsz$measured_value[jan_ftsz$measurement == "npq"] <- (jan_ftsz$measured_value[jan_ftsz$measurement == "npq"])+abs(min((filter(jan_ftsz, measurement == "npq"))$measured_value))
...
```{r}
jan_ftsz <- add_number(jan_ftsz)
jan_ftsz <-add_day_col(jan_ftsz)
```
...
```{r jan_ftsz_outliers}
jan_ftsz_outliers <- remove_outliers(jan_ftsz)
###Number of outliers
sum(is.na(jan_ftsz_outliers$measured_value))
###Proportion of measured values that are outliers
sum(is.na(jan_ftsz_outliers$measured_value))/nrow(jan_ftsz_outliers)
...
jan_ftsz_p <- p_value(jan_ftsz)

###Test 1 
unique(filter(jan_ftsz_p, genotype == "ftsz2-1", time_point == "1", measurement == "npq")$p)
a <- filter(jan_ftsz, genotype == "ftsz2-1", time_point == "1", measurement == "npq")$measured_value
b <- filter(jan_ftsz, genotype == "Col0", time_point == "1", measurement == "npq")$measured_value
wilcox.test(a, b, correct = FALSE)$p.value
###Test 2
unique(filter(jan_ftsz_p, genotype == "ftsz2-2", time_point == "3", measurement == "leafarea")$p)
c <- filter(jan_ftsz, genotype == "ftsz2-2", time_point == "3", measurement == "leafarea")$measured_value
d <- filter(jan_ftsz, genotype == "Col0", time_point == "3", measurement == "leafarea")$measured_value
...
```{r jan_ftsz_correct_p_values}
jan_ftsz_p_corrected <- corrected_p_value(jan_ftsz_p)
jan_ftsz_p_corrected$effect_size <- factor(jan_ftsz_p_corrected$effect_size, levels = c("small", "medium", "large"))
summary(jan_ftsz_p_corrected$effect_size)
```

Here is the distribution of the effect sizes - note that there are no "large" effect sizes, and no effect sizes are inf.  

```{r jan_ftsz_investigate_effect}
ggplot(data = subset(jan_ftsz_p_corrected, !is.na(effect_size)), aes(x = effect_size, na.rm = TRUE))+
  geom_bar()+
  labs(x = "Effect size", y = "Count", title = "January: Effect size for significant p-values, for Ftsz")+
  theme_minimal(base_family = "Calibri",
              base_size = 50
              )
#ggsave("jan_ftsz_effect_size.png", scale = 4)
```

Next, look at summary statistics for the p-values:

```{r jan_ftsz_summary_p}
###Summary statistics for the p values for each measurement;leaf area is never significant
jan_ftsz_p_corrected%>%group_by(measurement, type)%>%summarize(median = median(p), min = min(p), max = max(p))
...
jan_ftsz_p_corrected%>%group_by(type)%>%summarize(p_lessthan_0.05 = sum(p<0.05), p_lessthan_0.01 = sum(p<0.01))

```

Next, we will visualize p-values for January for Ftsz. Create a modified data frame to be used in to create visualizations.

```{r jan_ftsz_create_plot_data}
jan_ftsz_plot <- add_number(jan_ftsz_p_corrected)
...
ggplot(jan_ftsz_plot, aes(x=p, color=type)) +
  ...
  theme_minimal(base_family = "Calibri",
              base_size = 50)+
  labs(x = "P-Value", y = "Count", title = "January: P-value Distribution, for Ftsz")

#ggsave("jan_ftsz_adjusted_unadjusted_p_values.png", scale = 4)
```
...
Add bins for the p-values to easily visualize what is significant and what isn't. The minimum p-value for leafarea is greater than 0.05. For this experiment, leaf area is significantly different for both the p and adjusted p values - this was not the case for the January and January experiments.  


```{r jan_ftsz_plot_subsets}
###Create seperate data frames for each measurement to use in heat maps; don't use adjusted p-values

jan_ftsz_npq <- filter(jan_ftsz_plot, measurement == "npq", type == "p")%>%
...
jan_ftsz_phi2 <- filter(jan_ftsz_plot, measurement == "phi2", type == "p")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
jan_ftsz_leafarea <- filter(jan_ftsz_plot, measurement == "leafarea", type == "p")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
```{r jan_ftsz_modify_bins}

###In order to use these bins as the fill in a heat map, convert to a factor
jan_ftsz_npq$bin <- as.factor(jan_ftsz_npq$bin)
jan_ftsz_phi2$bin <- as.factor(jan_ftsz_phi2$bin)
jan_ftsz_leafarea$bin <- as.factor(jan_ftsz_leafarea$bin)


###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
jan_ftsz_npq$bin <- relevel(jan_ftsz_npq$bin, 'p<0.01')
jan_ftsz_phi2$bin <- relevel(jan_ftsz_phi2$bin, 'p<0.01')
jan_ftsz_leafarea$bin <- relevel(jan_ftsz_leafarea$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
jan_ftsz_npq$genotype <- reorder(jan_ftsz_npq$genotype, jan_ftsz_npq$number)
jan_ftsz_phi2$genotype <- reorder(jan_ftsz_phi2$genotype, jan_ftsz_phi2$number)
jan_ftsz_leafarea$genotype <- reorder(jan_ftsz_leafarea$genotype, jan_ftsz_leafarea$number)
...
```{r jan_ftsz_uncorrected_p_value_heat_maps}

##### ----- Jan Ftsz p-value heat map - NPQ ------
ggplot(data = jan_ftsz_npq, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: NPQ P-value, Uncorrected, for Ftsz")+
...
#ggsave("jan_ftsz_npq_uncorrected.png", scale = 4)

##### ----- Jan Ftsz p-value heat map - Phi2 ------
ggplot(data = jan_ftsz_phi2, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: Phi2 P-value, Uncorrected, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_manual(values = c("red", "orange", "black"))
#ggsave("jan_ftsz_phi2_uncorrected.png", scale = 4)

##### ----- Jan Ftsz p-value heat map - Leafarea ------
ggplot(data = jan_ftsz_leafarea, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwithout FDR Correction", x = "Hours", y = NULL, title = "January: Leafarea P-value, Uncorrected, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_manual(values = c("red", "orange", "black"))
#ggsave("jan_ftsz_leafarea_uncorrected.png", scale = 4)
...
jan_ftsz_phi2_adj <- filter(jan_ftsz_plot, measurement == "phi2", type == "p_adj")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
jan_ftsz_leafarea_adj <- filter(jan_ftsz_plot, measurement == "leafarea", type == "p_adj")%>%
  ###Create a new column with the p-value "bins"
  mutate(bin = case_when(
    (p <0.01)~ 'p<0.01',
    (p>0.01 & p < 0.05)~ "0.01<p<0.05",
...
```{r jan_ftsz_plot_modify_bins_adj}
###In order to use these bins as the fill in a heat map, convert to a factor
jan_ftsz_npq_adj$bin <- as.factor(jan_ftsz_npq_adj$bin)
jan_ftsz_phi2_adj$bin <- as.factor(jan_ftsz_phi2_adj$bin)
jan_ftsz_leafarea_adj$bin <- as.factor(jan_ftsz_leafarea_adj$bin)

###We want p<0.1 to be first in the legend, so refactor with p<0.01 as the first term
jan_ftsz_npq_adj$bin <- relevel(jan_ftsz_npq_adj$bin, 'p<0.01')
jan_ftsz_phi2_adj$bin <- relevel(jan_ftsz_phi2_adj$bin, 'p<0.01')
jan_ftsz_leafarea_adj$bin <- relevel(jan_ftsz_leafarea_adj$bin, 'p<0.01')

###Reorder by number so heat map has WT first, then single, then double mutants
jan_ftsz_npq_adj$genotype <- reorder(jan_ftsz_npq_adj$genotype, jan_ftsz_npq_adj$number)
jan_ftsz_phi2_adj$genotype <- reorder(jan_ftsz_phi2_adj$genotype, jan_ftsz_phi2_adj$number)
jan_ftsz_leafarea_adj$genotype <- reorder(jan_ftsz_leafarea_adj$genotype, jan_ftsz_leafarea_adj$number)
...
```{r}
jan_ftsz_plot%>%filter(type == "p_adj")%>%group_by(measurement)%>%summarize(sum(p<0.01), sum(p>0.01 & p<0.05))
```
...
##### ----- Jan Ftsz p-value heat map - NPQ ------
ggplot(data = jan_ftsz_npq_adj, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwith FDR Correction", x = "Hours", y = NULL, title = "January: NPQ P-value, Corrected, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_manual(values = c("red", "orange", "black"))
#ggsave("jan_ftsz_npq_corrected.png", scale = 4)
##### ----- Jan Ftsz p-value heat map - NPQ ------
ggplot(data = jan_ftsz_phi2_adj, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwith FDR Correction", x = "Hours", y = NULL, title = "January: Phi2 P-value, Corrected, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_manual(values = c("red", "orange", "black"))
#ggsave("jan_ftsz_phi2_corrected.png", scale = 4)
##### ----- Jan Ftsz p-value heat map - Leafarea ------
ggplot(data = jan_ftsz_leafarea_adj, aes(x = time_point, y = genotype, fill = bin)) + 
  labs(fill = "P-Value, \nwith FDR Correction", x = "Hours", y = NULL, title = "January: Leafarea P-value, Corrected, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_manual(values = c("red", "orange", "black"))
#ggsave("jan_ftsz_leafarea_corrected.png", scale = 4)

```


## Cell 371
Next, we'll transition to general visualizations for January for the Ftsz genotypes. 

Apply the cell_371_data function to the Ftsz January subset, and add columns with the day, number, and number_2.

```{r jan_ftsz_cell_371}
cell_371_jan_ftsz <- cell_371_data(add_day_col(jan_ftsz))
cell_371_jan_ftsz$genotype <- as.character(cell_371_jan_ftsz$genotype)
cell_371_jan_ftsz <- add_number(cell_371_jan_ftsz)
#cell_371_jan_ftsz$day <- as.integer(cell_371_jan_ftsz$day)

```

Next, we want to verify that our function is giving us the correct log fold values.

```{r jan_ftsz_verify_median}
###Test 1 
a <- filter(jan_ftsz, genotype == "ftsz2-2", time_point == "1", measurement == "npq")$measured_value
b <- filter(jan_ftsz, genotype == "Col0", time_point == "1", measurement == "npq")$measured_value
c <- log2(median(a/median(b)))
unique(c)
unique(filter(cell_371_jan_ftsz, genotype == "ftsz2-2", time_point == "1", measurement == "npq")$log2_fold)
###Test 2 
d <- filter(jan_ftsz, genotype == "ftsz-dbl", time_point == "1", measurement == "phi2")$measured_value
e <- filter(jan_ftsz, genotype == "Col0", time_point == "1", measurement == "phi2")$measured_value
f <- log2(median(d/median(e)))
f
unique(filter(cell_371_jan_ftsz, genotype == "ftsz-dbl", time_point == "1", measurement == "phi2")$log2)
...
Begin by creating heat maps using the data from the Cell 371 preperation. Create subsets of the January data based on measurement, and reorder each of the subsets by the number. 

```{r jan_ftsz_cell_371_subsets}
cell_371_leafarea_jan_ftsz <- cell_371_jan_ftsz%>%filter(measurement == "leafarea")%>%arrange(number_2)
cell_371_npq_jan_ftsz <- cell_371_jan_ftsz%>%filter(measurement == "npq")%>%arrange(number_2)
cell_371_phi2_jan_ftsz<- cell_371_jan_ftsz%>%filter(measurement == "phi2")%>%arrange(number_2)

###Reorder each measurement data set by the number column so Col0 is on top, followed by single, followed by double mutants
cell_371_leafarea_jan_ftsz$genotype <- reorder(cell_371_leafarea_jan_ftsz$genotype, cell_371_leafarea_jan_ftsz$number_2)
cell_371_npq_jan_ftsz$genotype <- reorder(cell_371_npq_jan_ftsz$genotype, cell_371_npq_jan_ftsz$number_2)
cell_371_phi2_jan_ftsz$genotype <- reorder(cell_371_phi2_jan_ftsz$genotype, cell_371_phi2_jan_ftsz$number_2)

```

```{r jan_ftsz_logfold_summary}
summary(cell_371_jan_ftsz$log2_fold)
```

Next, plot the data. Some notes: I chose to set the color scale to be in the range of -1.35 to 0.9 for all measurements. I chose the same scale so that all three heat maps were comparable. This encompasses the range of the log fold change values for January. 

```{r jan_ftsz_logfold_heat_maps}
##### ----- Jan Ftsz Phi2 heat map - all genotypes ------

ggplot(data = filter(cell_371_phi2_jan_ftsz, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "January: Phi2 Log 2 Fold Change, for Ftsz")+
  #geom_tile(width = 10 , height = 20)+
  ...
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.75, 1.35), breaks = c(-0.75, 0, 1.35), labels = c("-0.75", "0", "1.35"))
#ggsave("jan_ftsz_phi2_heatmap.png", scale = 4)

##### ----- Jan Ftsz Leaf area heat map - all genotypes ------

###Confirm that each day has two "cells" - this is because we selected only start and end points for each day
ggplot(data = filter(cell_371_leafarea_jan_ftsz, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "January: Leaf Area Log 2 Fold Change, for Ftsz")+
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.75, 1.35), breaks = c(-0.75, 0, 1.35), labels = c("-0.75", "0", "1.35"))
#ggsave("jan_ftsz_leafarea_heatmap.png", scale = 4)

##### ----- Jan Ftsz NPQ heat map - all genotypes ------

ggplot(data = filter(cell_371_npq_jan_ftsz, genotype != "Col0"), aes(x = time_point, y = genotype, fill = log2_fold)) + 
  labs(fill = "Log 2 Fold Change", x = "Hours", y = NULL, title = "January: NPQ Log 2 Fold Change, for Ftsz")+
  geom_tile(width = 10 , height = 20)+
  facet_grid(genotype ~ day, scales = "free", switch = "y")+
  #scale_x_continuous(breaks =
  #round(c(0,15,24,39.5,48,63.7,72,87,96,112,120,135,144,159,168,
  ...
  theme(strip.background.y = element_blank(),
        ...
        panel.spacing=unit(0, "lines"))+
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-0.75, 1.35), breaks = c(-0.75, 0, 1.35), labels = c("-0.75", "0", "1.35"))
#ggsave("jan_ftsz_npq_heatmap.png", scale = 4)
...
Apply the cell_370_data_function to the Janraury data, and add columns with the day, number, and number_2.

```{r jan_ftsz_cell_370}
cell_370_data_jan_ftsz <- cell_370_data(add_day_col(jan_ftsz))
cell_370_data_jan_ftsz$genotype <- as.character(cell_370_data_jan_ftsz$genotype)
cell_370_data_jan_ftsz <- add_number(cell_370_data_jan_ftsz)
cell_370_data_jan_ftsz <- add_day_col(cell_370_data_jan_ftsz)

cell_370_data_jan_ftsz$genotype <- reorder(cell_370_data_jan_ftsz$genotype, cell_370_data_jan_ftsz$number)
...
```{r jan_ftsz_cell_370_plots}
ggplot(data = cell_370_data_jan_ftsz, aes(x = time_point, y = med))+
...
#ggsave("jan_ftsz_median_plots.png", scale = 4)
```
