---
title: "DEPI Analysis - December MPK"
...
The data was collected in December 2017 and Februrary 2018. These different collection periods will be analyzed seperately in seperate RMD files.
...
```{r load_depi_data}
depi_data <-read.table("C:/Users/Owner/Documents/Research/Shiu_Lab/Shiu_Lab_R/Data/DEPI_Dec_Feb.txt", sep = ",", header = FALSE)
head(depi_data)
```
...
dec_data <- add_day_col(add_number(dec_data))
```
...
This proportion is similar to the propotion of outliers in the February data set.
...
###Test 1 
unique(filter(dec_data_p, genotype == "mpk1", time_point == "1", measurement == "npq")$p)
...
###Test 2
unique(filter(dec_data_p, genotype == "mpk14-17", time_point == "0", measurement == "leafarea")$p)
c <- filter(dec_data, genotype == "mpk14-17", time_point == "0", measurement == "leafarea")$measured_value
d <- filter(dec_data, genotype == "Col0", time_point == "0", measurement == "leafarea")$measured_value
wilcox.test(c,d, correct = FALSE)$p.value
###Test 3
unique(filter(dec_data_p, genotype == "mpk13", time_point == "0", measurement == "phi2")$p)
...
summary(dec_data_p_corrected$effect_size)
```

Notice that some effect sizes are infinity. This applies to all p-values of 1, because qnorm(1) = infinity. 

Here is the distribution of the effect sizes and the rows that contain infinite effect sizes. 

```{r dec_investigate_effect}
ggplot(data = subset(dec_data_p_corrected, !is.na(effect_size)), aes(x = effect_size, na.rm = TRUE))+
  geom_bar()+
  labs(x = "Effect size", y = "Count", title = "December: Effect size for significant p-values")+
  theme_minimal(base_family = "Calibri",
              base_size = 50)

dec_data[(which(dec_data_p_corrected$effect==Inf)),]
```
...
dec_data_p_corrected%>%group_by(measurement, type)%>%summarize(median = median(p), min = min(p), max = max(p))
...
Next, we will visualize p-values for December. Create a modified data frame to be used in to create visualizations.

```{r dec_create_plot_data}
...
ggplot(dec_data_plot, aes(x=p, color=type)) +
  geom_histogram(position="stack", fill = "white", size = 5)+
  ...
  theme_minimal(base_family = "Calibri",
              base_size = 50)+
  labs(x = "P-Value", y = "Count", title = "December: P-value Distribution")
...
Add bins for the p-values to easily visualize what is significant and what isn't. The minimum p-value for leafarea is greater than 0.05. **No leaf area measurement is significantly different from wildtype, so we don't need to include a heat map for leaf area.**
...
```{r dec_cell_371}
cell_371_dec <- cell_371_data(add_day_col(dec_data))
...
unique(c)
unique(filter(cell_371_dec, genotype == "mpk1", time_point == "1", measurement == "npq")$log2_fold)
###Test 2 
d <- filter(dec_data, genotype == "mpk9-16", time_point == "1", measurement == "phi2")$measured_value
e <- filter(dec_data, genotype == "Col0", time_point == "1", measurement == "phi2")$measured_value
f <- log2(median(d/median(e)))
f
unique(filter(cell_371_dec, genotype == "mpk9-16", time_point == "1", measurement == "phi2")$log2)
...
Next, plot the data. Some notes: I chose to set the color scale to be in the range of -1.5 to 0.8 for all measurements. I chose the same scale so that all three heat maps were comparable. This encompasses the range of the log fold change values for December. 

```{r dec_logfold_heat_maps}
...
ggplot(data = cell_371_phi2_dec, aes(x = time_point, y = genotype, fill = log2_fold)) + 
  ...
  scale_fill_gradient2(low = "blue", high="red", mid = "white", midpoint = 0, limits = c(-1.5, .8), breaks = c(-1.5, 0, 0.8), labels = c("-1.5", "0", "0.8"))
...
```{r dec_cell_370}
cell_370_data_dec <- cell_370_data(add_day_col(dec_data))
cell_370_data_dec <- add_number(cell_370_data_dec)
cell_370_data_dec <- add_day_col(cell_370_data_dec)
cell_370_data_dec$genotype <- reorder(cell_370_data_dec$genotype, cell_370_data_dec$number)
```

```{r dec_cell_370_loop}
for (element in genotype_combinations){
  data <- filter(cell_370_data_dec, genotype %in% c(element, "Col0"))
