...
data <- read.csv("~/Research/Shiu_Lab/Shiu_Lab_R/Data/Fitness_NormalizedValues_OutliersIncluded_07302021.csv", header = TRUE, row.names =  1)
### Remove mpk5_17
data <- data%>%
  filter(Genotype != "mpk5_17")
...
test$NormalizedSN <- SN_normalize[,1][1:482]
...
NormalizedID <- SN.temp%>%
  arrange(ID, NormalizedExp)%>%
  select(ID)
MeasuredValue_ID <- SN.temp%>%
  arrange(ID, Measured_Value)%>%
  select(ID)
### This is true for 100% of the IDs
sum(NormalizedID[,1] == MeasuredValue_ID[,1]) / nrow(SN.temp)
...
write.csv(NormalizedData_byExp, file = "Fitness_NormalizedValuesByExp_OutliersIncluded_07302021.csv")
